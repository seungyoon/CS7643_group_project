# https://raw.githubusercontent.com/openai/neural-gpu/master/neuralgpu/generators.py
...
class DataGenerator(object):
  ...
  #nclass = 33
  nclass = 11 #Yoon for reverse
  name = '<unknown task>'
  taskid = 0
  height = None
  min_length = 1

  #random.seed('20210416')
...
class FGenerator(DataGenerator):
  ...
  def rand_pair(self, l):
    ...
    x = np.random.randint(self.nclass - 1, size=l) # Yoon, rev to be 0 - 9
...
class CopyGenerator(DataGenerator): # Yoon for copy task
  def __init__(self, base):
    self.base = base

  def rand_pair(self, l):
    x = [np.random.randint(self.base) for _ in range(l)]
    inp = x
    res = x
    return inp, res
